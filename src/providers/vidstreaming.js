...
import { generateEncryptAjaxParameters, decryptEncryptAjaxResponse } from '../extractors/vidstreaming.js'

const USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0 Win64 x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.4692.71 Safari/537.36'

const BASE_URL = 'https://goone.pro'
...
export async function getVideoAnime(epId) {
    const url = `${BASE_URL}/videos/${epId}`
...
export async function scrapeM3U8(episodeId) {
  ...
  try {
    ...
    if (episodeId) {
      episodePage = await axios.get(`${BASE_URL}/videos/${episodeId}`)
...
export async function searchAnime(query) {
    return fetch(`${BASE_URL}/search.html?keyword=${query}`)
        .then(response => {
            return response.text()
        })
        .then(html => {
